...
export default function MazeCanvas({ fase }) {
  // pegando o valor passado pelo o pai, App.jsx
  console.log(fase)
    ...
    <div id='maze-complete' style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: '12px' }}>
      <div id="maze-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div>
          ...
          <h2 style={{ margin: '0', fontSize: '24px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '8px' }}>{fase_atual.label || 'Labirinto 01'}</h2>
        </div>
        <span style={{ backgroundColor: '#2e3347', padding: '4px 12px', borderRadius: '999px', fontSize: '13px', color: '#9ca3af' }}>
          {fase_atual.tipo}
        </span>
      </div>
      <div id="maze-draw">
