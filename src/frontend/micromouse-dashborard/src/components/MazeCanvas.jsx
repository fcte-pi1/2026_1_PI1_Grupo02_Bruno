...
export default function MazeCanvas({ tamanhoGrid, fase, grid, setGrid, setRastro, mouseX, mouseY, rastro }) {
    ...
    useEffect(() => {
        ...
        rastro.forEach(({ x, y, direcao }) => {
            ...
            // apaga a seta antiga redesenhando o fundo da célula
            const xCelula = offsetX + x * tamanho
            const yCelula = offsetY + (dimensao - 1 - y) * tamanho
            ctx.fillStyle = '#11131f'
            ctx.fillRect(xCelula, yCelula, tamanho, tamanho)

            // redesenha as paredes da célula (para não apagar as paredes)
            ...
            if (celula) {
                ...
                if (temParede(celula.parede_norte)) {
                    ctx.beginPath()
                    ctx.moveTo(xCelula, yCelula)
                    ctx.lineTo(xCelula + tamanho, yCelula)
                    ctx.stroke()
                }
                if (temParede(celula.parede_sul)) {
                    ctx.beginPath()
                    ctx.moveTo(xCelula, yCelula + tamanho)
                    ctx.lineTo(xCelula + tamanho, yCelula + tamanho)
                    ctx.stroke()
                }
                if (temParede(celula.parede_leste)) {
                    ctx.beginPath()
                    ctx.moveTo(xCelula + tamanho, yCelula)
                    ctx.lineTo(xCelula + tamanho, yCelula + tamanho)
                    ctx.stroke()
                }
                if (temParede(celula.parede_oeste)) {
                    ctx.beginPath()
                    ctx.moveTo(xCelula, yCelula)
                    ctx.lineTo(xCelula, yCelula + tamanho)
                    ctx.stroke()
                }
            }

            // agora desenha a nova seta
